...
Class: ECE6122
Last Date Modified: Sep 30, 2019
Description:
...
#pragma omp parallel for reduction(max : maxProduct) // omp parallization 
    // loop through the rows. 
    for (long long i = 0; i < nRows; i++)  
    { 
#pragma omp parallel for private(result)  // omp parallelization
...
int main(int argc, char** argv)
{
    ...
    maxProduct = maxAdjProduct(matrix); // call function to compute maximum adjacent product
